...
const utils = {
    deletePuppy: function(id, fnc) {
        ...
        fs.readFile(fileName, utf8, (err, contents) => {
            if (err) {
                ...
                const oldPuppyIndex = jsonObj.puppies.findIndex((ele) => ele.id === id)
    ...
    uploadFile: function(file, fnc) {
        ...
        file.mv(uploadFilePath, function(err) {
            if (err) {
    ...
    updatePuppy: function(newPuppy, fnc) {
        ...
        fs.readFile(fileName, utf8, (err, contents) => {
            if (err) {
                ...
                const oldPuppyIndex = jsonObj.puppies.findIndex((ele) => ele.id === newPuppy.id)
                jsonObj.puppies[oldPuppyIndex] = {...jsonObj.puppies[oldPuppyIndex], ...newPuppy }
                    // avoiding index
                    // let oldPuppy = jsonObj.puppies.find((ele) => ele.id == newPuppy.id)
